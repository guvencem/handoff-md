...
import { AssembledContext, FormatLevel, estimateTokens } from './assembler';
...
program
  ...
  .action(async (repoPath: string, options: any) => {
    ...
    const format: FormatLevel = ['compact', 'standard', 'full'].includes(options.format)
      ? options.format as FormatLevel
      : 'standard';
    ...
    // Render
    const output = renderHandoff(ctx);
