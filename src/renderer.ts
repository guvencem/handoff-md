import { AssembledContext, truncateToTokenBudget, getBudget } from './assembler';
...
export function renderHandoff(ctx: AssembledContext): string {
  ...
  if (ctx.structure.conventions.length > 0 || ctx.config) {
    ...
    sections.push(truncateToTokenBudget(convSection, budget.conventions + (ctx.config ? budget.config : 0)));
  }
