...
function analyzeStack(cwd) {
    ...
    if (pkg) {
        ...
        else if (hasDep(pkg, 'hono'))
            info.framework = 'Hono';
        else if (hasDep(pkg, 'elysia'))
            info.framework = 'Elysia';
        else if (hasDep(pkg, 'astro'))
            info.framework = 'Astro';
        else if (hasDep(pkg, 'gatsby'))
            info.framework = 'Gatsby';
        else if (hasDep(pkg, 'electron'))
            info.framework = 'Electron';
        else if (hasDep(pkg, 'react-native'))
            info.framework = 'React Native';
        else if (hasDep(pkg, 'expo'))
            info.framework = 'Expo';
    }
    // Python frameworks
    if (info.language === 'Python') {
        ...
        try {
            ...
            if (reqs.includes('django') || reqs.includes('Django'))
                ...
                info.framework = 'FastAPI';
            else if (reqs.includes('flask') || reqs.includes('Flask'))
                info.framework = 'Flask';
        }
        catch { }
    }
    ...
    if (info.language === 'Go') {
        try {
            ...
            if (gomod.includes('gin-gonic'))
                info.framework = 'Gin';
            else if (gomod.includes('gofiber'))
    ...
    if (fileExists(cwd, 'prisma', 'schema.prisma') || hasDep(pkg, 'prisma')) {
        ...
        try {
            ...
            else if (schema.includes('mongodb'))
    ...
    if (!info.database) {
        if (hasDep(pkg, 'pg') || hasDep(pkg, 'postgres'))
            ...
            info.database = 'SQLite';
        else if (hasDep(pkg, 'redis') || hasDep(pkg, 'ioredis'))
            info.database = 'Redis';
        else if (fileExists(cwd, 'supabase'))
    ...
    if (fileExists(cwd, 'vercel.json') || fileExists(cwd, '.vercel'))
        info.deploy = 'Vercel';
    else if (fileExists(cwd, 'netlify.toml'))
        info.deploy = 'Netlify';
    else if (fileExists(cwd, 'fly.toml'))
        info.deploy = 'Fly.io';
    else if (fileExists(cwd, 'railway.json') || fileExists(cwd, 'railway.toml'))
        info.deploy = 'Railway';
    else if (fileExists(cwd, 'Dockerfile') || fileExists(cwd, 'docker-compose.yml') || fileExists(cwd, 'docker-compose.yaml'))
        info.deploy = 'Docker';
    else if (fileExists(cwd, 'serverless.yml'))
        info.deploy = 'Serverless';
    else if (fileExists(cwd, 'amplify.yml'))
        info.deploy = 'AWS Amplify';
    // Testing detection
